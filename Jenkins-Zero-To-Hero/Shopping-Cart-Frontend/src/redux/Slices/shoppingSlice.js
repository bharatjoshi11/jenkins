...
export const getToCart = createAsyncThunk(
  ...
  async (items, {rejectWithValue}) => {
    try {
      ...
      const response = await axios.post('/api/v1/cart/get-cart', 
      items, {
        headers: {
...
export const updateCartItem = createAsyncThunk(
  ...
  async (value, {rejectWithValue}) => {
    try {
      const token = localStorage.getItem("accessToken");
      const response = await axios.patch(`/api/v1/cart/update-cart`, 
      value, {
        headers: {
          'authorization': token,
        }
...
export const deleteCartItem = createAsyncThunk(
  ...
  async (value, {rejectWithValue}) => {
    try {
      const token = localStorage.getItem("accessToken");
      const response = await axios.delete(`/api/v1/cart/delete-cart`, {
        data : value,
        headers: {
          'authorization': token,
        }
